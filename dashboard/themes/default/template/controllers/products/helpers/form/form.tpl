	...
	<div class="row">
		<div class="productTabs col-lg-2 col-md-3">
			...
			{foreach $product_tabs key=numStep item=tab}
				{if $tab.name != "Pack"}
					<a class="list-group-item {if $tab.selected}active{/if}" {if $tab.id == "VirtualProduct"}style="display: none"{/if} id="link-{$tab.id}" href="{$tab.href|escape:'html':'UTF-8'}&amp;updateproduct">{$tab.name}</a>
				{/if}
			{/foreach}
			</div>
		</div>
