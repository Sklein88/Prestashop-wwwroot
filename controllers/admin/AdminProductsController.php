...
class AdminProductsControllerCore extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->_join .= ' JOIN `'._DB_PREFIX_.'product_shop` sa ON (a.`id_product` = sa.`id_product` AND sa.id_shop = '.$id_shop.')
				...
				LEFT JOIN `'._DB_PREFIX_.'employee` e ON (e.`id_employee` = a.`user_created`)
				LEFT JOIN `'._DB_PREFIX_.'product_download` pd ON (pd.`id_product` = a.`id_product`)';

        $this->_select .= 'shop.`name` AS `shopname`, a.`id_shop_default`, ';
        $this->_select .= $alias_image.'.`id_image` AS `id_image`, CONCAT(e.`firstname`, " ", e.`lastname`)  AS `name_employee`, cl.`name` AS `name_category`, '.$alias.'.`price`, 0 AS `price_final`, a.`is_virtual`, pd.`nb_downloadable`, sav.`quantity` AS `sav_quantity`, '.$alias.'.`active`, IF(sav.`quantity`<=0, 1, 0) AS `badge_danger`';
        ...
        {
            $this->fields_list['name_employee'] = array(
                'title' => 'Created User',
                'align' => 'left'
            );
        }
...
//        );
    ...
    public function getList($id_lang, $orderBy = null, $orderWay = null, $start = 0, $limit = null, $id_lang_shop = null)
    {
        ...
        if(!$this->context->employee->isSuperAdmin()) {
            $this->_where = $this->_where . ' AND `user_created` = ' . $this->context->employee->id;
        }
    ...
    public function processAdd()
    {
        ...
        if ($this->object->add()) {
            ...
            $this->object->updateIsVirtual((int)$this->object->id, true);
            $this->addCombination($this->object);
    ...
    public function renderForm()
    {
        ...
        $this->tpl_form_vars['isPhotographer'] = (bool)$this->context->employee->isPhotographer();
        $this->tpl_form_vars['isSuperAdmin'] = (bool)$this->context->employee->isSuperAdmin();
    ...
    public function addCombination(Product $product)
    {
        /*
        ...
        */
        ...
        $product->reference = sprintf("%06d", $product->id);

        // Size S
        $id_product_attribute = $product->addCombinationEntity(
            0, //Tools::getValue('attribute_wholesale_price'),
            0, //Tools::getValue('attribute_price') * Tools::getValue('attribute_price_impact'),
            0, //Tools::getValue('attribute_weight') * Tools::getValue('attribute_weight_impact'),
            0, //Tools::getValue('attribute_unity') * Tools::getValue('attribute_unit_impact'),
            0, //Tools::getValue('attribute_ecotax'),
            0,
            null, //Tools::getValue('id_image_attr'),
            $product->reference . "_S", //Tools::getValue('attribute_reference'),
            null,
            null, //Tools::getValue('attribute_ean13'),
            null, //Tools::getValue('attribute_default'),
            null, //Tools::getValue('attribute_location'),
            null, //Tools::getValue('attribute_upc'),
            null, //Tools::getValue('attribute_minimal_quantity'),
            array(),
            null //Tools::getValue('available_date_attribute')
        );
        StockAvailable::setProductDependsOnStock((int)$product->id, $product->depends_on_stock, null, (int)$id_product_attribute);
        StockAvailable::setProductOutOfStock((int)$product->id, $product->out_of_stock, null, (int)$id_product_attribute);

        $combination = new Combination((int)$id_product_attribute);
        $combination->setAttributes(array('1'));
        ...
        $id_product_attribute = $product->addCombinationEntity(
            ...
            0,
            null, //Tools::getValue('id_image_attr'),
            $product->reference . "_M", //Tools::getValue('attribute_reference'),
            null,
            null, //Tools::getValue('attribute_ean13'),
            null, //Tools::getValue('attribute_default'),
            null, //Tools::getValue('attribute_location'),
        ...
        $combination = new Combination((int)$id_product_attribute);
        $combination->setAttributes(array('2'));
        ...
        $id_product_attribute = $product->addCombinationEntity(
            ...
            0,
            null, //Tools::getValue('id_image_attr'),
            $product->reference . "_L", //Tools::getValue('attribute_reference'),
            null,
            null, //Tools::getValue('attribute_ean13'),
            null, //Tools::getValue('attribute_default'),
            null, //Tools::getValue('attribute_location'),
        ...
        $combination = new Combination((int)$id_product_attribute);
        $combination->setAttributes(array('3'));
    }
    ...
    */
    ...
    public function ajaxProcessaddProductImage()
    {
        ...
        $this->addImagesForeach($files, $legends, $product);

        die(Tools::jsonEncode(array($image_uploader->getName() => $files)));
    }


    public function addImagesForeach($files, $legends, $product)
    {
