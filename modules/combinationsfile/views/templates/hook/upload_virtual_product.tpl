...
<div class="form-group">
	<div class="col-lg-12" id="{$id|escape:'htmlall':'UTF-8'}-images-thumbnails">
		...
		<div class="img-thumbnail text-center">
			<p>{$file.image|escape:'htmlall':'UTF-8'}</p>
			{if isset($file.size)}<p>{l s='File size' mod='combinationsfile'} {$file.size|escape:'htmlall':'UTF-8'}kb</p>{/if}
			{if isset($file.delete_url)}
			<p>
				<a class="btn btn-default" href="{$file.delete_url|escape:'htmlall':'UTF-8'}">
					<i class="icon-trash"></i> {l s='Delete' mod='combinationsfile'}
				</a>
			</p>
			{/if}
		</div>
...
<div class="row">
	<div class="alert alert-warning">{l s='You have reached the limit (%s) of files to upload, please remove files to continue uploading' sprintf=$max_files mod='combinationsfile'}</div>
</div>
{else}
<div class="form-group">
	<div class="col-lg-12">
		...
		<div class="dummyfile input-group">
			...
			<span class="input-group-btn">
				<button id="{$id|escape:'htmlall':'UTF-8'}-selectbutton" type="button" name="submitAddAttachments" class="btn btn-default">
					<i class="icon-folder-open"></i> {if isset($multiple) && $multiple}{l s='Add files' mod='combinationsfile'}{else}{l s='Add file' mod='combinationsfile'}{/if}
				</button>
				...
				<a href="{$files[0].download_url|escape:'htmlall':'UTF-8'}">
					<button type="button" class="btn btn-default">
						...
						{if isset($size)}{l s='Download current file (%skb)' sprintf=$size mod='combinationsfile'}{else}{l s='Download current file' mod='combinationsfile'}{/if}
					</button>
				</a>
				{/if}
			</span>
	...
	$(document).ready(function(){
		...
		$('#{$id|escape:'htmlall':'UTF-8'}').change(function(e) {
			if ($(this)[0].files !== undefined)
			{
				...
				{*var filenName = $('#reference').val() + '_L' ;//+ $('#id_product_attribute_size').val();*}
		...
		if (typeof {$id|escape:'htmlall':'UTF-8'}_max_files !== 'undefined')
		{
			$('#{$id|escape:'htmlall':'UTF-8'}').closest('form').on('submit', function(e) {
				if ($('#{$id|escape:'htmlall':'UTF-8'}')[0].files.length > {$id|escape:'htmlall':'UTF-8'}_max_files) {
					e.preventDefault();
					alert('{l s='You can upload a maximum of %s files'|sprintf:$max_files mod='combinationsfile'}');
				}
			});
		}
	});
