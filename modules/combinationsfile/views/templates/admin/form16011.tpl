...
{block name="defaultForm"}
  ...
  <div class="productTabs col-lg-2">
    <div class="list-group">
      {foreach $product_tabs key=numStep item=tab}
        {if $tab.name != "Pack"}
          <a class="list-group-item {if $tab.selected}active{/if}" id="link-{$tab.id|escape:'htmlall':'UTF-8'}" href="{$tab.href|escape:'html':'UTF-8'}&amp;updateproduct">{$tab.name|escape:'htmlall':'UTF-8'}</a>
        {/if}
      {/foreach}
    </div>
  </div>
  ...
  var product_type_virtual = {Product::PTYPE_VIRTUAL|escape:'htmlall':'UTF-8'};
  var product_type_simple = {Product::PTYPE_SIMPLE|escape:'htmlall':'UTF-8'};

  var has_combinations = {$has_combinations|escape:'htmlall':'UTF-8'};
...
//      $('a[id*="Combinations"]').hide();
  ...
  <form id="product_form" class="form-horizontal col-lg-10" action="{$form_action|escape:'html':'UTF-8'}" method="post" enctype="multipart/form-data" name="product" novalidate>
    ...
    {foreach $product_tabs key=numStep item=tab}
      {if $tab.id != "Pack" }
        <div id="product-tab-content-{$tab.id|escape:'htmlall':'UTF-8'}" class="{if !$tab.selected}not-loaded{/if} product-tab-content" {if !$tab.selected}style="display:none"{/if}>
          {if $tab.selected}
            {$custom_form}
          {/if}
        </div>
