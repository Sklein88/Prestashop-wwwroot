...
<script type="text/javascript">
	var newLabel = '{l s='New label' mod='combinationsfile'}';
	var choose_language = '{l s='Choose language:' mod='combinationsfile'}';
	var required = '{l s='Required' mod='combinationsfile'}';
	var customizationUploadableFileNumber = '{$product->uploadable_files|escape:'html':'UTF-8'}';
	var customizationTextFieldNumber = '{$product->text_fields|escape:'html':'UTF-8'}';
	var uploadableFileLabel = 0;
	var textFieldLabel = 0;
</script>
<div id="product-virtualproduct" class="panel product-tab">
	<input type="hidden" name="submitted_tabs[]" value="VirtualProduct" />
	<input type="hidden" id="virtual_product_filename" name="virtual_product_filename" value="{$product->productDownload->filename|escape:'html':'UTF-8'}" />
	<h3>{l s='Virtual Product (services, booking or downloadable products)' mod='combinationsfile'}</h3>
	<div class="is_virtual_good" class="form-group">
		...
		<label for="is_virtual_good" class="t bold">{l s='Is this a virtual product?' mod='combinationsfile'}</label>
	</div>
		...
		<div class="form-group">
			...
			<div class="col-lg-2">
				<span class="switch prestashop-switch">
					...
					<label for="is_virtual_file_on">{l s='Yes' mod='combinationsfile'}</label>
					<input type="radio" name="is_virtual_file" id="is_virtual_file_off" value="0" {if !$product_downloaded} checked="checked"{/if} />
					<label for="is_virtual_file_off">{l s='No' mod='combinationsfile'}</label>
		...
		<div id="is_virtual_file_product" style="display:none;">
			...
			<div class="form-group">
				<div class="col-lg-push-3 col-lg-9">
					<div class="alert alert-danger" id="file_missing">
						{$download_product_file_missing|escape:'html':'UTF-8'} :<br/>
			...
			{if true}
				{if $product->productDownload->id}
					<input type="hidden" id="virtual_product_id" name="virtual_product_id" value="{$product->productDownload->id|escape:'intval'}" />
				{/if}
				<div class="form-group"{if $is_file} style="display:none"{/if}>
					<label id="virtual_product_file_label" for="virtual_product_file" class="control-label col-lg-3">{l s='File' mod='combinationsfile'}</label>
					<div class="col-lg-5">
				...
				<div class="form-group">
					...
					<div class="col-lg-5">
						<a href="{$product->productDownload->getTextLink(true)|escape:'html':'UTF-8'}" class="btn btn-default"><i class="icon-download"></i> {l s='Download file' mod='combinationsfile'}</a>
						<a href="{$currentIndex|escape:'html':'UTF-8'}&amp;deleteVirtualProduct=true&amp;updateproduct&amp;token={$token|escape:'html':'UTF-8'}&amp;id_product={$product->id|escape:'intval'}" class="btn btn-default" onclick="return confirm('{l s='Do you really want to delete this file?' js=1 mod='combinationsfile'}');"><i class="icon-trash"></i> {l s='Delete this file' mod='combinationsfile'}</a>
					</div>
				</div>
				{/if}
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-lg-5">
						<input class="datepicker" type="text" id="virtual_product_expiration_date" name="virtual_product_expiration_date" value="{$product->productDownload->date_expiration|escape:'html':'UTF-8'}" size="11" maxlength="10" autocomplete="off" />
			...
			{else}
				<div class="alert alert-info">
					{l s='You cannot edit your file here because you used combinations. Please edit this file in the Combinations tab.' mod='combinationsfile'}
				</div>
	...
	<div class="panel-footer">
		<a href="{$link->getAdminLink('AdminProducts')|escape:'html':'UTF-8'}" class="btn btn-default"><i class="process-icon-cancel"></i> {l s='Cancel' mod='combinationsfile'}</a>
