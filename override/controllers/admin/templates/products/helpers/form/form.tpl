			...
			setup : function(ed) {
				...
				ed.on('keydown', function(ed, e) {
					...
					if (max != 'none')
					{
						...
						rest = max - count;
						if (rest < 0)
							textarea.parent('div').find('span.counter').html('<span style="color:red;">{l s='Maximum' mod='combinationsfile'} '+ max +' {l s='characters' mod='combinationsfile'} : '+rest+'</span>');
						else
	...
	<div class="row">
		<div class="productTabs col-lg-2 col-md-3">
			...
			{foreach $product_tabs key=numStep item=tab}
				{if $tab.name != "Pack"}
					<a class="list-group-item {if $tab.selected}active{/if}" style="display: none" id="link-{$tab.id}" href="{$tab.href|escape:'html':'UTF-8'}&amp;updateproduct">{$tab.name}</a>
				{/if}
			{/foreach}
			</div>
		</div>
		<script type="text/javascript">
			...
			var empty_pack_msg = '{l s='This pack is empty. You will need to add at least one product to the pack before you can save.' js=1 mod='combinationsfile'}';
			var empty_name_msg = '{l s='The product name is empty. You will at least need to enter a name for the default language before you can save the product.' js=1 mod='combinationsfile'}';
			var empty_link_rewrite_msg = '{l s='The friendly URL is empty. You will at least need to enter a friendly URL for the default language before you can save the product.' slashes=1 mod='combinationsfile'}';
			var reload_tab_title = '{l s='Confirmation' slashes=1 mod='combinationsfile'}';
			var reload_tab_description = '{l s='A server error occurred while loading the tabs: some tabs could not be loaded.' js=1 mod='combinationsfile'}'+'\n'+'{l s='Please try again by refreshing the page.' js=1 mod='combinationsfile'}'+'\n'+'{l s='If you are still encountering this problem, please check your server logs or contact your hosting provider for assistance.' js=1 mod='combinationsfile'}';

			$('#product-tab-content-wait').show();
			var post_data = {$post_data};
			var save_error = {if $save_error}true{else}false{/if};
			var error_heading_msg = '{l s='Error' js=1 mod='combinationsfile'}';
			var error_continue_msg = '{l s='Continue' js=1 mod='combinationsfile'}';
			...
			$(document).ready(function()
			{
				...
				$('.confirm_leave').live('click', function(){
					...
					return confirm("{l s='You will lose all unsaved modifications. Are you sure that you want to proceed?' js=1 mod='combinationsfile'}");
				});
		...
		<script type="text/javascript">
			// <![CDATA[
				ThickboxI18nImage = "{l s='Image' mod='combinationsfile'}";
				ThickboxI18nOf = "{l s='of' mod='combinationsfile'}";
				ThickboxI18nClose = "{l s='Close' mod='combinationsfile'}";
				ThickboxI18nOrEscKey = "{l s='(or "Esc")' mod='combinationsfile'}";
				ThickboxI18nNext = "{l s='Next >' mod='combinationsfile'}";
				ThickboxI18nPrev = "{l s='< Previous' mod='combinationsfile'}";
				tb_pathToImage = "../img/loadingAnimation.gif";
			//]]>
		</script>

		<div id="product-tab-content-wait" style="display:none">
			<div id="loading"><i class="icon-refresh icon-spin"></i>&nbsp;{l s='Loading...' mod='combinationsfile'}</div>
		</div>
		...
		<form id="product_form" class="form-horizontal col-lg-10 col-md-9" action="{$form_action|escape:'html':'UTF-8'}" method="post" enctype="multipart/form-data" name="product" novalidate>
			...
			<div class="alert alert-info draft" >
				{l s='Your product will be saved as a draft.' mod='combinationsfile'}
				<a href="#" class="btn btn-default" onclick="submitAddProductAndPreview()" ><i class="icon-eye-open"></i>&nbsp;{l s='Save and preview' mod='combinationsfile'}</a>
