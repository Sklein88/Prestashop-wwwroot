    ...
    public function updateDownloadProduct($product, $edit = 0)
  {
    if (Tools::getValue('is_virtual_good_combination') == 'true')
    {
      ...
      if (isset($_FILES['virtual_product_file_uploader_combination']) && $_FILES['virtual_product_file_uploader_combination']['size'] > 0)
      {
        ...
        //[START] Upload cover image cropped
        $filesToProcess = $helper->process();

        $separeteByDot = explode(".", $filesToProcess[0]['tmp_name']);
        $new_tmp_name = dirname($filesToProcess[0]['tmp_name']) . '\\' . basename($separeteByDot[0]) . '_c.tmp';

        $newThumb = Image::CroppedThumbnail($filesToProcess[0]['tmp_name'], 67, 45);
        imagejpeg($newThumb, $new_tmp_name);
        imagedestroy($newThumb);

        $filesToProcess[0]['tmp_name'] = $new_tmp_name;

        $this->addImagesForeach($filesToProcess, array($virtual_product_filename), $product);
        //[END] Upload cover image cropped
